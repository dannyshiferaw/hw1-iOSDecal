...
func arePalindromes(_ words: [String]) -> Bool {
    // your code here
    for word in words {
        if !isPalindrome(word) { return false }
    }
    return true
}
...
func isPalindrome(_ word: String) -> Bool {
    var count = 0
    var offset = -1
    while (count < word.count && offset >= -word.count) {
        let isSame = word[word.index(word.startIndex, offsetBy: count)] ==
            word[word.index(word.endIndex, offsetBy: offset)]
...
func findStudent(withStudentID sid: Int?, studentIDNumbers: [Int: String] = getSIDsDictionary()) -> String? {
    ...
    // your code here
    if let suspectSID = sid {
        if hasEightDigits(suspectSID) {
            for (id, studentName) in studentIDNumbers {
                if (id == suspectSID) { return "\(studentName) stole the cookie!" }
            }
        }
    }
    return nil
}
//returns true if a given number has eight digits
func hasEightDigits(_ num: Int) -> Bool {
    var count = 0
    var n = num
    while(n > 0) {
        n = n / 10
...
//test hasEightDigits
hasEightDigits(11111111) == true
hasEightDigits(1111111) == false
...
class Building {
    ...
    func getCapacity() -> Int {
        if let cap = self.capacity { return cap }
        return 0
    }
}

extension Building: Equatable {
    static func == (this: Building, other: Building) -> Bool {
...
var percentageStringClosure = { String($0*10.0) + "%" }
//: Tests below (Ignore)
